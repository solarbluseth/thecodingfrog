...
else
{
	...
	newTextLayer.textItem.contents = paragraph;
	newTextLayer.textItem.position = Array(10, 10);
	newTextLayer.textItem.width = 300;
...
function getLoremIpsum()
{
	...
	if (socket.open("www.lipsum.com:80"))
	{
		socket.write("GET /feed/html?start=no HTTP/1.1\nHost: www.lipsum.com\n\n");
		var html = socket.read(9999999);
		socket.close();
		//alert(html);
		
		var re = new RegExp("\n*<p>\n*.*","gim")
		
		var texte = html.match(re);
		if (texte != null)
		{
			//alert(texte.length);
			//alert(texte[0]);
			content = texte[0].substring(5);
		}
		else
		{
			content = "";
